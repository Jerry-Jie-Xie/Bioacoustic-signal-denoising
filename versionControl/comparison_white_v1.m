...
noiseReductionMethods = {'Berouti', 'Boll',...
    ...
    'Towsey', 'WaveletStudentT'};

% noiseReductionMethods = {'MMSESTSA'};
%=========Whistle=============%
...
if nWinLen == 1
    figure;
    plot(improvedSNRmat, 'LineWidth',1);
    ...
    for i = 1:N
        plot(improvedSNRmat(:,i), plotPattern{i}, 'LineWidth',2);
        hold on;
    end
    legend(noiseReductionMethods);   
end
title('Eastern Yellow Robin, SNR: -10, White Noise')
xlabel('Window size (samples)');
xticklabels({'256', '', '512', '', '1024', '', '2048', '', '4096'});
ylabel('SNR improvement');
print(fig,'Eastern_Yellow_Robin_SNR_-10_White_Noise','-dpng')
