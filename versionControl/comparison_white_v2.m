...
close all; clear; clc;
%======================%
noiseReductionMethods = {'Towsey1', 'Berouti', 'Boll',...
    'MultiBandKamath', 'MMSESTSA', 'Wiener',...
    'WaveletStudentT'};

% noiseReductionMethods = {'WaveletAllFrames'};
%=========Whistle=============%
...
for iMethod = 1:nMethods
    ...
    snrValueArray = [-10,-5,0,5];
    nSnr = length(snrValueArray);
    diffSNR_array = zeros(nSnr,1);
    for iSnr = 1:nSnr
        ...
        for iWin = 1:nWinLen
            ...
            switch noiseReductionMethod
                ...
                case 'Towsey1'
                    ...
                    Y=fft(y,nfft);
                    YPhase=angle(Y(1:floor(length(Y)/2)+1,:)); %Noisy Speech Phase
                    Y=abs(Y(1:floor(length(Y)/2)+1,:)).^Gamma; %Specrogram
                ...
                case 'WaveletSingleFrame'
                    
                    noiseReducedSignal = WaveletSingleFrame(noisedSignal,fs,IS,winLen,SP);
        ...
        diffSNR_array(iSnr) = enhancedSNR_array;
...
title('Eastern Yellow Robin, White Noise')
xlabel('Input SNR (dB)');
xticklabels({'-10', '', '-5', '', '0', '', '5'});
ylabel('Output SNR (dB)');
print(fig,'Eastern_Yellow_Robin_White_Noise','-dpng')
