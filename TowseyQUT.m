...
function I3 = TowseyQUT(I1,rT)
%=========%
[M,N] = size(I1);
% calculate the noise profile for each frequency bin
% noise_base = rep(0,M);
noise_base = zeros(M,1);
pitch = round(N/4);
%=========%
% the recording is longer than 2 seconds
frame_base = round (2/rT);
if pitch > frame_base
   pitch = 200;
else
   pitch = frame_base;
...
for nf = 1:M
    ...
    [~,index] = max(counts);   
    if index > pitch*0.98
        bandNoise = mid(round(0.98*pitch));
    else
        bandNoise = mid(index);                
    end    
...
% smooth noiseBase
smoothNoiseBase = smooth(noise_base,7);
% remove noiseBase
I2 = zeros(M,N);
for n = 1:M 
...
bin = floor(max(M/32+1,3));
frame = floor(max(bin/3,3));
I3 = zeros(M,N);
for i = 0:(frame-1)
    for j = 0:(bin-1)       
